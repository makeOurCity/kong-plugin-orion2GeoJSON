...
if [ -x "$SCRIPT_DIR/01_configure_kong.sh" ]; then
    if ! "$SCRIPT_DIR/01_configure_kong.sh"; then
        echo -e "${RED}エラー: Kongの設定に失敗しました${NC}"
        exit 1
    fi
else
    chmod +x "$SCRIPT_DIR/01_configure_kong.sh" && "$SCRIPT_DIR/01_configure_kong.sh"
    if [ $? -ne 0 ]; then
        echo -e "${RED}エラー: Kongの設定に失敗しました${NC}"
        exit 1
    fi
